...
export type OverlayDetailLevel = 'collapsed' | 'compact';
...
export const OVERLAY_CYCLE_HOTKEY = 'Super+Shift+D';

/** Human-readable labels for Settings hints. */
export const OVERLAY_TOGGLE_HOTKEY_LABEL = 'Ctrl+Shift+O';
export const OVERLAY_CYCLE_HOTKEY_LABEL = 'Shift+Win+D';

/**
 * Always-visible on-HUD shortcut strip (compact only — collapsed is too small).
 * Uses the shared cycle label so Settings and HUD stay in sync.
 */
export const OVERLAY_SHORTCUT_HINT_LABEL = `O dismiss · ${OVERLAY_CYCLE_HOTKEY_LABEL} denser`;
...
export const OVERLAY_SIZES: Record<OverlayDetailLevel, { width: number; height: number }> = {
  ...
  /** Compact includes a one-line shortcut footer (Shift+Win+D hint). */
  compact: { width: 260, height: 86 },
};

export function isOverlayDetailLevel(value: unknown): value is OverlayDetailLevel {
  return value === 'collapsed' || value === 'compact';
}

/**
 * Normalize persisted/runtime values. Legacy `expanded` maps to `compact`.
 * Unknown values fall back to the default.
 */
export function coerceOverlayDetailLevel(value: unknown): OverlayDetailLevel {
  if (isOverlayDetailLevel(value)) return value;
  if (value === 'expanded') return 'compact';
  return DEFAULT_OVERLAY_DETAIL_LEVEL;
}

/** Advance one step: collapsed ↔ compact. */
