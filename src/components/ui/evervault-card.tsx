...
export const EvervaultCard = ({
  ...
  const maskImage = useTransform(() => {
    const x = mouseX.get();
    const y = mouseY.get();
    return `radial-gradient(250px at ${x}px ${y}px, white, transparent)`;
  });
...
export function CardPattern({ randomString, maskImage }: CardPatternProps) {
  return (
    <div className="pointer-events-none absolute inset-0 z-0">
      <div className="absolute inset-0 rounded-2xl opacity-0 [mask-image:linear-gradient(white,transparent)] group-hover/card:opacity-50 transition-opacity duration-500" />
      <motion.div
...
export const Icon = ({ className, ...rest }: React.SVGAttributes<SVGSVGElement>) => (
  <svg
