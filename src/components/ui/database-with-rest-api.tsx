...
const DatabaseWithRestApi = ({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-[300px] w-full max-w-[500px] flex-1 flex-col items-center min-w-0",
        className
      )}
    >
      <svg
      ...
      <div className="absolute bottom-10 flex w-full flex-col items-center min-w-0">
        <div className="absolute -bottom-4 h-[100px] w-[62%] rounded-lg bg-accent/30" />
        <div className="absolute -top-3 z-20 flex items-center justify-center rounded-lg border bg-[#101112] px-2 py-1.5 sm:-top-4 sm:py-1.5 max-w-[95%] min-w-0">
          <Sparkles className="size-3 flex-shrink-0" />
          <span className="ml-2 text-sm truncate" title={title ?? "Taste DNA from your activity"}>
            {title ?? "Taste DNA from your activity"}
          </span>
        </div>
        <div className="absolute -bottom-8 z-30 grid h-[60px] w-[60px] place-items-center rounded-full border-t bg-[#141516] font-semibold text-sm text-white/80">
          {circleIcon ?? (circleText ? circleText : <User className="size-7" />)}
        </div>
        <div className="relative z-10 flex h-[150px] w-full items-center justify-center overflow-hidden rounded-lg border bg-background shadow-md min-w-0">
          <div className="absolute bottom-8 left-10 z-10 flex h-8 max-w-[50%] items-center gap-2 rounded-full border bg-[#101112] px-3 text-sm min-w-0">
            <HeartHandshake className="size-4 flex-shrink-0" />
            <span className="truncate">{buttonTexts?.first ?? "Genome"}</span>
          </div>
          <div className="absolute right-10 z-10 hidden h-8 max-w-[50%] items-center gap-2 rounded-full border bg-[#101112] px-3 text-sm sm:flex min-w-0">
